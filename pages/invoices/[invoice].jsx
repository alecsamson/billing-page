...
export default function Invoice(props) {
  ...
  const myInvoice =
    props.state.length > 0
      ...
      : 0;
  console.log("my invoice is " + myInvoice);
  const limitDate = props.state.length > 0 ? myInvoice.date : 0;
  const splitDate = props.state.length > 0 ? myInvoice.date.split("/") : 0;
  const initialDate =
    props.state.length > 0
      ? `${splitDate[0]}/0${splitDate[1] - 1}/${splitDate[2]}`
      : 0;
